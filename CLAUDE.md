...
| 단어 암기 · TOEFL Voca 01 | `voca:1-1/1-2/2-1/2-2` | 80 (20 / 20 / 20 / 20) |
| 해커스 보카 Day 1 | `hk:1/2/3` | 65 (26 / 20 / 19) |

위 표(Voca+Day1) 145장 + (D) 확장(Day2 63·Day3 62·경선식 18) = **해커스 계열(hackers) 총 288장**.
...
- **정답 공개 후 문장 속 단어를 탭하면 클립보드 복사**(`makeAnswerCopyable`/`copyText`, 슬롯·너비 공용). ⚠️ 너비 모드 입력창은 `disabled`면 click이 안 떠서 **`readOnly`로** 둔다. **★`clearCopyable`은 readOnly를 절대 풀지 않는다(항상 `true` 유지)** — 예전엔 여기서 `false`로 풀어 정답칸이 편집 가능해졌고, 그 탓에 한/영 IME가 한글 조합을 밀어넣어 backspace가 자모 단위로 지워지는 버그(2026-07-14)가 있었다. 피드백의 정답(`.copy-b`)도 위임 클릭으로 복사. `hFeedbackHTML`은 hFeedback의 innerHTML 버전.
...
## 8. 지켜야 할 규칙 & 함정 요약
0. **입력 = 강제 영어(한영키 무관)**: 빈칸 input 3종은 **항상 `readonly`**(HTML 속성 + `clearCopyable`도 `true` 유지 — ★readOnly를 절대 false로 풀지 말 것, 풀면 IME 조합이 살아나 backspace 자모삭제 버그 재발). 값은 `kbdKey`로만 주입. 2차 안전망으로 3종에 `compositionstart→preventDefault+blur`. 물리키 핸들러는 **`e.code`(물리 위치)** 로 판별해 IME가 한글모드여도 영문만 입력 — `e.key`(회귀 금지). 입력 라우팅은 `kbInput` 변수로만 하므로 DOM 포커스는 불필요. 전역 keydown이라 **어디를 클릭했든** 정답칸으로 감. 온스크린 키는 **`pointerdown`**(즉시 발동, `click`은 터치 연타 유실 → 회귀 금지). `.key`에 `touch-action:manipulation`.
...
- **★빈 진도 덮어쓰기 방지**: `cloudPushNow`는 `store.words`가 비고 `clearedAt`도 없으면(로드 실패·최초 로딩 등 비정상) push를 건너뜀 → 타 기기 진도가 빈 데이터로 날아가지 않게. 진짜 초기화는 `words={}`라도 `clearedAt`이 있어 정상 전파.
- **★이탈/종료 시 즉시 플러시**(`cloudFlush`): `visibilitychange`(hidden)·`pagehide`에서 대기중 1.5s 디바운스를 취소하고 `cloudPushNow({keepalive:true})` 즉시 호출. 답하고 바로 앱을 나가/닫아도 클라우드 미반영 유실 방지. keepalive는 flush 경로에서만(언로드 중 완료 보장, body<64KB). `sbRpc(fn,body,keepalive)` 3번째 인자.
...
- 연습 모드: 주제/유형(일상·의견·⭐)/랜덤, 자유 녹음(45초 초과 시 주황), 재생·재녹음·별표(`malhaeboca_speak_stars`). **질문 표시/숨김 토글(👁/🙈, `settings.spkShowQ` 기본 표시)** — 숨김이면 🔊로 듣고 답하는 연습(문항만 임시로 여는 "질문 보기"), 숨김모드에선 출처 배지도 가림. **녹음 종료 후 결과 블록에 질문 전문 + 출처 `Set X-Y · Qnn` 공개**(교재 대조용, 파일명에도 포함).
- **교재 출처 라벨**: `SPK_SETS`(주제별 personal/opinion이 어떤 Set을 이어붙였는지 개수로 기술) + `spkSrcLabel(topicId,type,qi)` → `speakingPool`이 각 문항에 `src` 부착. **8주제 전부 Set 1~8**(living=Set1[1-1 11·1-2 11·1-3 7·1-4 8], career=Set2[2-1~2-4 각8], tech=Set3 …). 라벨 형식 `Set X-Y · Qnn`, **Qnn은 주제 전체 연번**(교재 방식: personal 세트들 먼저→opinion, 예 지역사회 8-1~8-4=Q01~Q32, 8-4=Q25~Q32; 주거 1-1~1-4=Q01~Q37). 주제칩엔 `spkTopicSetNo`로 Set 번호 배지. **문항 추가 시 `SPK_SETS` 개수도 같이 갱신**(안 하면 이후 라벨 어긋남). 스모크에 세트경계·주제연번·270개 단언.
