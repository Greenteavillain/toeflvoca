...
- **교재 출처 라벨**: `SPK_SETS`(주제별 personal/opinion이 어떤 Set을 이어붙였는지 개수로 기술) + `spkSrcLabel(topicId,type,qi)` → `speakingPool`이 각 문항에 `src` 부착. 구 문항(living/career)은 `src=null`. **문항 추가 시 `SPK_SETS` 개수도 같이 갱신**(안 하면 이후 문항 라벨이 어긋남). 스모크에 세트경계·201개 단언.
