...
- ⚠️ TWA APK는 아직 netlify.app을 가리킴 → github.io 서브패스는 TWA 에셋링크(도메인 루트 필요)와 안 맞음. APK 갱신하려면 Cloudflare Pages(pages.dev 루트) 등으로 이전 후 재빌드 필요. 브라우저+홈화면추가(PWA)는 새 URL로 바로 됨.
