...
### 배포 (Netlify, 2026-07-10)
- 비공개 레포라 **Netlify 직접 배포**(git 자동배포 아님). 라이브: **https://toeflvoca.netlify.app** (site id `e348d1cb-d755-4e6a-a290-c3fabfc2d91c`, 팀 `greenteavillain`).
- 재배포: 레포 루트에서 **`bash deploy.sh`**. 앱 파일만 임시 스테이징 후 `netlify deploy --dir … --prod`. **빌드 없음 = 무료 빌드분 0 소모**(자주 업데이트해도 제한 안 걸림). CLI는 `npx -y netlify-cli`로 즉석 사용, 전역설치 없음.
- 토큰은 **레포 밖** `~/.config/toeflvoca/netlify_token`(600). 레포엔 site id만(비밀 아님). ⚠️ 절대 토큰을 레포/스테이징에 두지 말 것.
...
- **★주제 추가 = `SPEAKING_TOPICS` 배열에 객체 추가가 전부**: `{id, title, titleEn, scenario, personal:[..], opinion:[..]}`. 유저가 학원에서 새 유형 배워오면 여기에 얹는다. 스모크의 개수 단언(69문항 등)도 같이 갱신할 것.
